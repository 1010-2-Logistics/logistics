...
Gateway는 **외부에서 들어온 위 헤더를 화이트리스트 판정보다 먼저 무조건 제거**한다. 이 순서를 지키지 않으면 토큰 없이 헤더만 위조해도 통과된다.

**현재 범위의 한계 (별도 이슈 필요)**

- **Gateway 우회 방지는 네트워크에 의존한다.** 각 서비스는 여전히 헤더를 무조건 신뢰하므로, 서비스 포트에 직접 접근하면 헤더 위조가 통한다. 배포 환경에서는 Gateway(19091)만 열고 나머지는 도커 내부 네트워크에만 두어 차단하고 있다.
- **인가(role별 접근 제어)는 일부만 구현되어 있다.** `SecurityConfig`가 있는 서비스는 `user`·`company`·`product` 3개뿐이고, 나머지는 로그인한 사용자면 무엇이든 호출할 수 있다.
- 권한 문자열 규칙이 서비스마다 다르다 (`user`는 `ROLE_` 접두사 사용, `company`·`product`는 미사용).
